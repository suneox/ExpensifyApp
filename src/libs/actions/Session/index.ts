...
/**
 * Calls the LOG_OUT API to invalidate the session on the server.
 */
function signOut(params: {autoGeneratedLogin?: string; signedInWithSAML?: boolean; authToken?: string} = {}): Promise<void | Response<never>> {
    ...
    console.log('[SSO-DEBUG][ND] signOut()', {
        signedInWithSAML: params.signedInWithSAML,
        hasAuthToken: !!params.authToken,
        platform: getPlatform(),
        isHybridApp: CONFIG.IS_HYBRID_APP,
    });
    if (params.signedInWithSAML && params.authToken) {
        // eslint-disable-next-line no-console
        console.log('[SSO-DEBUG][ND] signOut -> SAML branch (callSAMLSignOut)');
        return callSAMLSignOut(logOutParams, params.authToken);
    }

    // eslint-disable-next-line no-console
    console.log('[SSO-DEBUG][ND] signOut -> regular LOG_OUT branch');
    // eslint-disable-next-line rulesdir/no-api-side-effects-method
    return API.makeRequestWithSideEffects(SIDE_EFFECT_REQUEST_COMMANDS.LOG_OUT, logOutParams, {});
}

function callSAMLSignOut(params: LogOutParams, authToken: string): Promise<void | Response<never>> {
    const isWeb = getPlatform() === CONST.PLATFORM.WEB;
    // Native includes `shouldUseMobileRedirect=1` to match OldDot's SSO logout URL
    // (Mobile-Expensify `constructURLForSSO`), which signals the backend to redirect back to
    // the app scheme. If the backend honors it, `openAuthSessionAsync` resolves `success` and
    // the web view self-closes; if not, the fire-and-forget + `dismissAuthSession()` fallback
    // below still completes sign-out (see #85937).
    const queryString = isWeb ? `referer=ecash&authToken=${authToken}` : `mobile=true&shouldUseMobileRedirect=1&appversion=${pkg.version}&referer=ecash&authToken=${authToken}`;
    const expectedURL = isWeb ? CONFIG.EXPENSIFY.NEW_EXPENSIFY_URL : CONST.SAML_REDIRECT_URL;
    const samlLogoutURL = `${CONFIG.EXPENSIFY.SAML_URL}/logout?${queryString}`;
    // eslint-disable-next-line no-console
    console.log('[SSO-DEBUG][ND] callSAMLSignOut start', {
        isWeb,
        samlLogoutURL,
        expectedURL,
        SAML_URL: CONFIG.EXPENSIFY.SAML_URL,
        NEW_EXPENSIFY_URL: CONFIG.EXPENSIFY.NEW_EXPENSIFY_URL,
        SAML_REDIRECT_URL: CONST.SAML_REDIRECT_URL,
    });
    // On native, the SAML Single Logout endpoint does NOT redirect back to our
    // `expensify://open` scheme — iOS `ASWebAuthenticationSession` only auto-dismisses when it
    // catches that scheme, so `openAuthSessionAsync` resolves only as `cancel` when the user
    // manually closes the web view (or via `dismissAuthSession()` below). To preserve ordering
    // (LOG_OUT must run AFTER the IdP SLO has had time to invalidate the session, to avoid the
    // race where the app signs out before the IdP session dies), `LOG_OUT` is dispatched inside
    // `.then`/`.catch` (fires once the web view closes). `dismissAuthSession()` auto-closes the
    // web view after a delay so the user does not have to dismiss it manually (see #85937).
    if (!isWeb) {
        // Open the SLO web view. LOG_OUT runs only after this resolves/closes (success redirect,
        // user cancel, or dismissAuthSession below) — preserving SLO-before-LOG_OUT ordering.
        openAuthSessionAsync(samlLogoutURL, expectedURL)
            .then((rs) => {
                // eslint-disable-next-line no-console
                console.log('[SSO-DEBUG][ND] SIGN_OUT result', rs);
                // eslint-disable-next-line no-console
                console.log('[SSO-DEBUG][ND] native -> dispatching LOG_OUT (after SLO resolved)');
                // eslint-disable-next-line rulesdir/no-api-side-effects-method
                return API.makeRequestWithSideEffects(SIDE_EFFECT_REQUEST_COMMANDS.LOG_OUT, params, {});
            })
            .catch((error) => {
                // eslint-disable-next-line no-console
                console.log('[SSO-DEBUG][ND] native SLO openAuthSessionAsync error (ignored)', {error: String(error)});
                Log.hmmm('SAML sign out failed', {error});
                // eslint-disable-next-line no-console
                console.log('[SSO-DEBUG][ND] native -> dispatching LOG_OUT (fallback after error)');
                // Fallback: still sign the user out of the app if the promise rejects.
                // eslint-disable-next-line rulesdir/no-api-side-effects-method
                return API.makeRequestWithSideEffects(SIDE_EFFECT_REQUEST_COMMANDS.LOG_OUT, params, {});
            });
        // Auto-close the SLO web view after giving the IdP enough time to invalidate the session.
        // The backend logout endpoint does not redirect to `expensify://`, so without this the web
        // view would stay open until the user manually dismisses it. `dismissAuthSession()`
        // resolves the pending promise with `{type: 'dismiss'}` → `.then` above runs `LOG_OUT`.
        Timers.register(
            setTimeout(() => {
                // eslint-disable-next-line no-console
                console.log('[SSO-DEBUG][ND] native -> dismissAuthSession() auto-closing SLO modal');
                dismissAuthSession();
            }, 1000 * 60 * 1), // auto close after 1 minute
        );
        return;
    }

    // Web: the popup redirects back to NEW_EXPENSIFY_URL and `maybeCompleteAuthSession()`
    // (App.tsx) closes it. Keep awaiting the result so a user-initiated cancel can abort.
    return openAuthSessionAsync(samlLogoutURL, expectedURL)
        .catch((error) => {
            // eslint-disable-next-line no-console
            console.log('[SSO-DEBUG][ND] openAuthSessionAsync threw', {error: String(error)});
            Log.hmmm('SAML sign out failed', {error});
        })
        .then((result) => {
            // eslint-disable-next-line no-console
            console.log('[SSO-DEBUG][ND] openAuthSessionAsync result', {
                type: result?.type,
                url: result && result.type === 'success' ? result.url : undefined,
            });
            if (result && result.type !== 'success') {
                // eslint-disable-next-line no-console
                console.log('[SSO-DEBUG][ND] REJECT path -> LOG_OUT NOT called', {resultType: result?.type});
            ...
            // eslint-disable-next-line no-console
            console.log('[SSO-DEBUG][ND] SUCCESS path -> calling LOG_OUT');
...
function signOutAndRedirectToSignIn(shouldResetToHome?: boolean, shouldStashSession?: boolean, shouldSignOutFromOldDot = true, shouldForceUseStashedSession?: boolean) {
    ...
    if (isSupportal && shouldStashSession && hasStashedSession(stashedSession, stashedCredentials)) {
        onyxSetParams = {
...
function signInWithShortLivedAuthToken(authToken: string, isSAML = false) {
    ...
    // eslint-disable-next-line no-console
    console.log('[SSO-DEBUG][ND] signInWithShortLivedAuthToken', {isSAML, authMethod, willSetSignedInWithSAML: isSAML});
